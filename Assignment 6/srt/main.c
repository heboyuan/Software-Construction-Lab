...
void *processingImage(void *nthread)
{
    Vec3 camera_pos;
    ...
    /* for every pixel */
    for( int px=*(int*)nthread; px<width; px += nthreads )
    {
        ...
        for( int py=0; py<height; ++py )
        {
            ...
            result[px][py][0] = scaled_color[0];
            result[px][py][1] = scaled_color[1];
            result[px][py][2] = scaled_color[3];
        }
    }
    return NULL;
}
...
main( int argc, char **argv )
{
    ...
    {
        pthread_create (&threads[x], NULL, (void *) &processingImage, (void *) &x);
    }
    ...
    for (int px=0; px < width; ++px)
    {
        for (int py=0; py < height; ++py)
        {
             ...
             */
            printf( "%.0f %.0f %.0f\n",
            result[px][py][0], result[px][py][1], result[px][py][2] );
        }
        printf( "\n" );
    }    
